...
include( '/home/soxred93/public_html/common/rfalib2.php');
$tool = 'RFAP';
...
$tools = mysql_connect("sql",$toolserver_username,$toolserver_password);
@mysql_select_db("u_soxred93", $tools) or toDie( "MySQL error, please report to X! using <a href=\"http://en.wikipedia.org/wiki/User:X!/Bugs\">the bug reporter.</a> Be sure to report the following SQL error when reporting:<br /><pre>".mysql_error()."</pre>" );
...
$query = "select page_latest,page_title,COUNT(*) from revision join page on page_id = rev_page where 
...
while ( $rfas = mysql_fetch_assoc($result) ) {
    ...
    $ts = "SELECT rev_timestamp FROM revision WHERE rev_id = '".$rfas['page_latest']."';";
...
foreach ($rfastoupdate as $rfatoup) {
    ...
    $md5 = md5($how2db);
    $existq = "DELETE FROM u_soxred93.rfap WHERE name = '$rfatoup';";
    $existr = mysql_query($existq, $tools);
    $insert = "INSERT INTO u_soxred93.rfap (name , md5 , pulls , data ) VALUES ( '$rfatoup' , '$md5', '0' , '$how2db' );";
...
foreach ($thisrfas as $key => $arfa) {
    ...
    foreach($data['support'] as $od) {
        ...
        if($od == $name_s || $od == $name_a || $od == $name_b || in_array('User:'.$od, $names_old)) {
            ...
            echo "<li><a href = \"http://en.wikipedia.org/wiki/Wikipedia:$arfa\">$stripped_arfa</a>$viewout</li>\n";
