...
if( !isset( $_GET['name'] ) ) {
	$msg = 'Welcome to X!\'s namespace counter tool!<br /><br />
		<form action="http://toolserver.org/~soxred93/topedits/index.php" method="get">
...
if( ISIPADDRESS == false ) {//IP addresses don't have a field in the user table, so IPs must be done the old way.
	...
	if( ISIPADDRESS == false ) {//IPs don't have a user ID
		$query = "SELECT /* SLOW_OK */ rev_timestamp,page_title,page_namespace,rev_comment,page_is_redirect FROM revision JOIN page ON page_id = rev_page WHERE rev_user = '".$uid."' AND page_namespace = '".$namespace."' ORDER BY rev_timestamp ASC;";
	}
	else {
		$query = "SELECT /* SLOW_OK */ rev_timestamp,page_title,page_namespace,rev_comment,page_is_redirect FROM revision JOIN page ON page_id = rev_page WHERE rev_user_text = '".$name."' AND page_namespace = '".$namespace."' ORDER BY rev_timestamp ASC;";
	}
...
foreach ( $unique_articles as $page => $arts ) {
	...
	print '<li>'.$arts." - <a href='http://en.wikipedia.org/wiki/".$nscolon.str_replace('_',' ',urlencode( $page )).'\'>';
