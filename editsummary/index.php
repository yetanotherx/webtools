...
function getNamespaces() {
	...
	$namespaces = $http->get( 'http://'.$oldlang.'.'.$oldwiki.'.org/w/api.php?action=query&meta=siteinfo&siprop=namespaces&format=php', false );
...
if( !isset( $_GET['name'] ) ) {
	toDie( 'Welcome to X!\'s summary calculator!<br /><br />
...
@mysql_select_db( 'toolserver' ) or toDie( "MySQL error, please report to X! using <a href=\"http://en.wikipedia.org/wiki/User:X!/Bugs\">the bug reporter.</a> Be sure to report the following SQL error when reporting:<br /><pre>".mysql_error()."</pre>" );
...
function getEditCounts() {
	...
	$query = "SELECT rev_comment,rev_timestamp,rev_minor_edit FROM revision JOIN page ON page_id = rev_page WHERE rev_user_text = '$name' AND page_namespace = 0 ORDER BY rev_timestamp DESC";
	$result = mysql_query($query);
	if(!$result) toDie( "MySQL error, please report to X! using <a href=\"http://en.wikipedia.org/wiki/User:X!/Bugs\">the bug reporter.</a> Be sure to report the following SQL error when reporting:<br /><pre>".mysql_error()."</pre>" );
...
echo "Username: <a href=\"http://$oldlang.$oldwiki.org/wiki/User:$name\">$name</a><br />";
